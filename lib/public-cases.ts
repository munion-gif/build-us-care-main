...
export type PublicCasesResult = {
  cases: PublicCaseItem[];
  count: number;
  facets: {
...
async function loadPublicCases(params: PublicCasesParams = {}): Promise<PublicCasesResult> {
  if (!hasSupabaseEnv()) {
    return {
      ...
      source: "empty_without_supabase"
    };
  }

  const service = params.service ?? "all";
  const region = params.region ?? "all";
  const limit = boundedNumber(params.limit, 20, 1, 50);
