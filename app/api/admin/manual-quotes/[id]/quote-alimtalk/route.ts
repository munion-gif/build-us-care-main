...
import { sendManualQuoteAlimtalk } from "@/lib/admin-quote-alimtalk";
...
export async function POST(request: Request, context: Context) {
  ...
  if (!parsedId.success) return fail("bad_request", "견적서 ID를 다시 확인해주세요.", 400);

  const result = await sendManualQuoteAlimtalk(parsedId.data);
  if (!result.ok) {
