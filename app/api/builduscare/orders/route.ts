...
async function assertSlotAvailable(params: {
  ...
  const manualQuoteCount = (manualQuotesResult.data ?? []).filter((quote) => quote.time_slot === timeSlot).length;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (reserved && slot) {
      const slotCap = await resolveRequestedSlotCap({ supabase, reservedDate: reserved, timeSlot: slot });
      if (slotCap.blocked) {
      ...
      const slotError = await assertSlotAvailable({ supabase, reservedDate: reserved, timeSlot: slot, cap: slotCap.cap });
