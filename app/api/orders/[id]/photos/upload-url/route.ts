...
export async function POST(request: Request, context: Context) {
  if (!hasSupabaseEnv()) {
    return fail("supabase_not_configured", "Supabase is required to create photo upload URLs.", 500);
  }
  ...
  const parsed = createPhotoUploadUrlSchema.safeParse(body);
  ...
  const filePath = createOrderPhotoPath(orderId.data, parsed.data.fileName, parsed.data.contentType);
