...
BEGIN
  ...
  IF p_status = 'confirmed' AND p_time_slot IN ('morning', 'afternoon') THEN
    v_cap := CASE
      WHEN p_time_slot = 'morning' THEN COALESCE(v_config.morning_cap, p_default_cap)
      ELSE COALESCE(v_config.afternoon_cap, p_default_cap)
    END;
