...
BEGIN
  ...
  v_next_order_status := CASE
    WHEN v_order_status IN ('paid', 'scheduled', 'in_progress', 'completed', 'done', 'canceled', 'issue', 'warranty') THEN v_order_status
    WHEN p_status = 'confirmed' THEN 'scheduled'::public.order_status
    ELSE v_order_status
  END;
